...
#    x.columns = ["chrnum", "from", "from.1", "chrnum.1", "to", "to.1", "value"]
    ...
    x[["from","to"]]=x[["from","to"]]*BINSIZE
    x.insert(0, "chrnum", "chr3", True)
    x.insert(2, "from.1", x['from']+BINSIZE, True)
    x.insert(3, "chrnum.1", "chr3", True)
    x.insert(5, "to.1", x['to']+BINSIZE, True)
